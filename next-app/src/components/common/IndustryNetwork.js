...
const CompanyCard = ({ company, isFeatured = false }) => {
  ...
  if (company.cardType === 'image' && company.logoFile) {
    try {
       const mod = require(`../../assets/images/company-logos/${company.logoFile}`);
       imageSrc = mod?.default?.src || mod?.src || (typeof mod?.default === 'string' ? mod.default : (typeof mod === 'string' ? mod : null)) || `/images/company-logos/${company.logoFile}`;
    } catch (e) {
       imageSrc = `/images/company-logos/${company.logoFile}`; 
    }
  }
